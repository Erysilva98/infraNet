...
export default function AppSistemas() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        if (allData && allData.sistemas) {
          setItems(allData.sistemas); // Assumindo que allData contém uma propriedade 'sistemas'
        }
      } catch (error) {
        console.log('Erro ao obter os dados no AppSistemas.jsx', error);
        setItems([]);
      }
    };
    fetchItems();
            ...
            <Link key={item.id} href={item.link || '/pages/404'}>
              <div>
                <div className="text-center flex flex-col flex-wrap-4 p-4">
                  ...
                  {/* Inclua aqui título, descrição, etc., se necessário */}
                </div>
              </div>
            </Link>
          ))}
